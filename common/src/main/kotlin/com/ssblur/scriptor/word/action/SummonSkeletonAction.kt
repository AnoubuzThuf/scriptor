  ...
  override fun apply(caster: Targetable, targetable: Targetable, descriptors: Array<Descriptor>, words: Array<Word?>) {
    ...
    if (caster is EntityTargetable) {
      if (caster.targetEntity is LivingEntity) {
        summoner = caster.targetEntity as LivingEntity
      }
    }
    ...
    summonedSkeleton.setSummonParams(summoner,  finalDuration, strength.toInt(), getColor(descriptors), behaviourDescriptors, level,
    ...
    level.gameEvent(GameEvent.ENTITY_PLACE, blockPos2,  GameEvent.Context.of(summoner))
    summonedSkeleton.setPos(vecPos)
  }
