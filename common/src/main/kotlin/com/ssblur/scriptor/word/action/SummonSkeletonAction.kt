...
import com.ssblur.scriptor.entity.SummonedSkeleton
  ...
  override fun apply(caster: Targetable, targetable: Targetable, descriptors: Array<Descriptor>, words: Array<Word?>) {
    ...
    if (caster is EntityTargetable) {
      if (caster.targetEntity is LivingEntity) {
        ...
        val summonedSkeleton: SummonedSkeleton? = SUMMONED_SKELETON.get().spawn(
        ...
        if (summonedSkeleton != null) {
          summonedSkeleton.moveTo(blockPos2, 0f, 0f)
          val tag = CompoundTag()
